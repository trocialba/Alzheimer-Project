---
title: "Differential abundance Status MCI vs HC"
author: "Alba Troci"
date: "`r Sys.Date()`"  
output:
...
```{r}
sample_data(ps3)$Alter <- as.factor(sample_data(ps3)$Alter)
sample_data(ps3)$Status <- as.factor(sample_data(ps3)$Status)
Group_DESeq <- phyloseq_to_deseq2(ps3, ~Alter+Status)
Group_DESeq
...
```{r}
res = results(ddsClean, contrast = c("Status", "MCI", "HC"))
```
...
export(sigtabgen, "mci.vs.hc.xlsx")
